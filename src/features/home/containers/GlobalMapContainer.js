...
const GlobalMapContainer = props => {
    ...
    const { vendorListRaw, siteListRaw } = state
    const clientLists = siteListRaw.reduce((r,c)=> {
        const index = r.reduce((r1,c1,i1) => c1.country===c.country && c1.city===c.city ? i1 : r1 , -1)
        if(r.length===0 || index===-1) {
            const sites = siteListRaw.filter( site => site.country===c.country && site.city===c.city)
                .map(site => ({ 
                    ...site,
                    name: site.site_name, 
