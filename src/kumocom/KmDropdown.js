...
const KmDropdown = props => {
    ...
    return (
        <div className="dropdown"
            style={{ cursor: 'pointer', background: style ? style.background : null, fontSize: 14, whiteSpace: 'nowrap' }}>
            <div className="d-flex  border-0 justify-content-center align-items-center"
                id="dropdownMenuButton" data-toggle="dropdown">
                {
                    selectedItem !== undefined && <span className="px-2" >{selectedItem.icon}</span>
                }
                {
                    selectedItem !== undefined && <span className="px-1 font-weight-bold">{selectedItem.text}</span>
                }
                <span className="dropdown-toggle pl-2 " style={{ color: 'gray', fontSize: 20 }}></span>
            </div>
